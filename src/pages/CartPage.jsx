      ...
      <div className="bg-background rounded-md w-full absolute h-fit min-h-screen">
        ...
        <div className="overflow-y-scroll p-4 w-full  grid grid-cols-2 md:grid-cols-3 ml-5 no-scroll">
          ...
          <div className="cursor-default md:absolute  top-20 mr-5 md:w-1/3  col-span-2 md:col-span-1  right-0 h-[calc(100%-90px)] text-text/50">
            <div className="sticky flex text-lg font-medium m-2 bg-backgrounds rounded-md min-h-56 h-fit flex-wrap text-wrap transition-all ease-in-out duration-700" style={{ top: `${top}px` }}>
              <div className="w-full p-2 px-4 border-b-2 border-text/10">
                <div className="flex w-full justify-between">
                  <div className="font-extrabold text-text">Products</div>
                  <div className="font-extrabold text-text">Price</div>
                  <div className="font-extrabold text-text">Quty.</div>
                </div>
                {prod.contents.data.map((item, i) => {
                  const isAdded = listt?.includes(item._id);
                  return isAdded ? (
                    <div key={item._id} className="flex w-full font-normal justify-between">
                      <div>{item.name}</div>
              ...
              <div className="w-full flex text-text flex-col justify-between p-4 pt-0">
                ...
                <button className="bg-primary mt-4 p-2 w-full rounded-md overflow-hidden h-10 group"><div className="group-hover:-translate-y-8 transition-all duration-500">Check-Out</div><div className="pt-1 transition-all duration-500 group-hover:-translate-y-8">Check-Out</div></button>
              </div>
            </div>
          </div>
        </div>
